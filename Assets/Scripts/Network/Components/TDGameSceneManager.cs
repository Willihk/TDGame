...
{
    public class TDGameSceneManager : MonoBehaviour, INetworkSceneManager
    {
        ...
        [SerializeField]
        private SceneChangeEvent _clientChangeScene = new SceneChangeEvent();

        [FormerlySerializedAs("ClientSceneChanged")]
        [SerializeField]
        private SceneChangeEvent _clientSceneChanged = new SceneChangeEvent();

        [FormerlySerializedAs("ServerChangeScene")]
        [SerializeField]
        private SceneChangeEvent _serverChangeScene = new SceneChangeEvent();

        [FormerlySerializedAs("ServerSceneChanged")]
        [SerializeField]
        private SceneChangeEvent _serverSceneChanged = new SceneChangeEvent();
        ...
        private AsyncOperation clientLoadingOperation;
        ...
        private void RegisterClientMessages(INetworkPlayer player)
        {
        ...
        internal void ClientSceneMessage(INetworkPlayer player, SceneMessage msg)
        {
            if (!Client.IsConnected)
            {
                throw new InvalidOperationException(
                    "ClientSceneMessage: cannot change network scene while client is disconnected");
            }

            if (string.IsNullOrEmpty(msg.scenePath))
            {
                throw new ArgumentNullException(nameof(msg.scenePath),
                    $"ClientSceneMessage: {nameof(msg.scenePath)} cannot be empty or null");
            }

            if (logger.LogEnabled())
                logger.Log($"ClientSceneMessage: changing scenes from: {ActiveScenePath} to: {msg.scenePath}");
        ...
        public void ChangeServerScene(string scenePath, SceneOperation sceneOperation = SceneOperation.Normal)
        {
            if (string.IsNullOrEmpty(scenePath))
            {
                throw new ArgumentNullException(nameof(scenePath),
                    "ServerChangeScene: " + nameof(scenePath) + " cannot be empty or null");
            }
        ...
        private UniTask ApplyOperationAsync(string scenePath, SceneOperation sceneOperation = SceneOperation.Normal)
        {
        ...
        private async UniTask ApplyNormalOperationAsync(string scenePath)
        {
        ...
        private async UniTask ApplyAdditiveLoadOperationAsync(string scenePath)
        {
            var scene = new AssetReference(scenePath);
        ...
        private async UniTask ApplyUnloadAdditiveOperationAsync(string scenePath)
        {
            var scene = new AssetReference(scenePath);
