...
namespace TDGame.Systems.Tower.Targeting.Systems
{
    public class ClosestTargetingSystem : SystemBase
    {
        ...
        protected override void OnUpdate()
        {
            ...
            var translations =
                enemyQuery.ToComponentDataArrayAsync<Translation>(Allocator.TempJob, out var translationHandle);
            ...
            var handle = job.Schedule(towerQuery, JobHandle.CombineDependencies(enemyHandle, translationHandle));
        ...
        private struct TargetJob : IJobEntityBatch
        {
            ...
            public NativeArray<Translation> EnemyTranslations;
            ...
            public void Execute(ArchetypeChunk batchInChunk, int batchIndex)
            {
                ...
                for (int i = 0; i < batchInChunk.Count; i++)
                {
                    ...
                    for (int j = 0; j < EnemyTranslations.Length; j++)
                    {
                        float distance = math.distance(translations[i].Value, EnemyTranslations[j].Value);
